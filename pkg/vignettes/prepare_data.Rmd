...
# Perform bm query
gene_ids_bm <- getBM(attributes = c("wormbase_gene",
                                 "ensembl_gene_id",
                                 "external_gene_name"), 
                     filters = "external_gene_name", # return matched gene names
                     values = head(gene_ids$Public.Name), # inputing public gene names
                     mart = mart)
head(gene_ids_bm)
```
...
We'll be fetching a GEO dataset and time series are good examples since we can easily check if age estimates are correct. 
...
mart <- useMart("ensembl", dataset = "celegans_gene_ensembl")
gene_coords <- getBM(attributes = c("wormbase_gene",
...
geo_hend <- getGEO("GSE52861", getGPL = F)[[1]]
pheno.hendriks <- pData(geo_hend)

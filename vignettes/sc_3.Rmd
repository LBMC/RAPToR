...
However, @francesconi2014effects later showed there is significant developmental spread between samples, spanning around 20 hours of 20°C late larval development, essentially making this data a very high-resolution time course hereafter called `dsrockman2010`.
...
Then, we restrict genes to the previously mentioned sets to estimate "**<span style="color:firebrick">Soma age</span>**" and "**<span style="color:royalblue">Germline age</span>**".

#### Validation

Since PCA or ICA components summarize the expression dynamics of the data, we can evaluate our results by plotting them along the different ages. 
...
We find our estimates match the previously inferred ages by @francesconi2014effects.
...
show_fig(expr = {
  plot(francesconi_time[dsrockman2010$p$geo_accession,"time"], ae_dsrockman2010$age.estimates[,1],
     ...
     xlab = "Francesconi & Lehner (2014) estimates, h past L4 stage (25°C)", ylab = "RAPToR (global) estimates, h past egg-laying (20°C)")
...
```
<br>
A lot of these components don't have a definite link to developmental processes, which is expected given the (intended) genetic background heterogeneity in the samples.
...
```{r sc3_cor_ae_prior, echo = F, fig.height=4, fig.width=5, fig.align='center', out.width="33%"}
show_fig(expr = {
...
par(mfcol = c(3,6))
  invisible(sapply(dev_comps, function(i){
    plot(ae_dsrockman2010$age.estimates[,1], ica_rock$M[,i], lwd = 2, col = "black",
     xlab = "Global age", ylab = "IC", main = paste0("(global) IC", i))
    
    plot(ae_soma_prior$age.estimates[,1], ica_rock$M[,i], lwd = 2, col = "firebrick",
         xlab = "Soma age", ylab = "IC", main = paste0("(soma w/ prior) IC", i))
    box(lwd = 2, col = "firebrick")
    
    plot(ae_germline$age.estimates[,1], ica_rock$M[,i], lwd = 2, col = "royalblue",
         xlab = "Germline age", ylab = "IC", main = paste0("(germline) IC", i))
    box(lwd = 2, col = "royalblue")
}))

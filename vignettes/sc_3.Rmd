...
```{r sc3_ica_allcomps, echo = F, fig.width=21, fig.height=12}
show_fig(expr = {
  par(mfrow = c(4,7), mar = c(4,4,3,1))
  invisible(sapply(seq_len(28), function(i){
    plot(ae_dsrockman2010$age.estimates[,1], ica_rock$M[,i], main = paste("IC", i), 
         ylab = "IC", xlab = "age", cex = .8)
  }))
}, fig.width=21, fig.height=12)
```
<br>
...
From the plots above, we can establish that 
 ...
 - component IC1 and IC9 are linked to oogenesis
 - component IC2 is clearly associated with spermatogenesis processes
 - components IC5, IC7 and IC8 have good contribution from the soma geneset, and show oscillatory dynamics.
...
We can notice on the soma estimates that a sample appears quite off from its global or germline age (it's marked with a $\small{\triangle}$).
...
Notice how components IC5, IC7 and IC8, previously associated with soma, are much cleaner when plotting with the soma age. 
Also, see that the IC1, IC2 and IC9 germline-associated components appear quite noisy.

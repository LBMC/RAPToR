...
```{r sc3_ql_icaplot, echo = F, fig.height=6, fig.width=15}
show_fig(expr = {
  par(mfrow = c(2,5))
...
From the plots above, we can establish that 
 ...
 - component IC1 is linked to oogenesis
 - components IC3 and IC5 have good contribution from the soma geneset.
 - component IC4's loadings suggest contribution from both the oogenesis gene set and the soma gene set.
...
Notice how components IC3 and IC5, previously associated with soma, are much cleaner when plotting with the soma age. 
Also, see that the IC1 and IC2 germline-associated components appear quite noisy.

...
heatmap(cor_dsbyrne2020[ord, ord], Colv = NA, Rowv = NA, 
        ...
        # RowSideColors = cols[as.numeric(dsbyrne2020$p$diet[ord])], 
        labRow = "", labCol = "")
par(xpd = T)
...
xlp <- 1.025
mtext(at = xlp, line = 4, side = 1, text = "(days)", cex = .8)
```

```{r ex4_corboxplot, fig.height=5, fig.width=8}
boxplot(cor_dsbyrne2020, xaxt="n",
        lwd=2, boxwex=.5, 
        col = 0, at = 1:sum(agetab)+rep(1:length(agetab), agetab),
        ylab = "Spearman correlation", xlab = "Chronological age (days of adulthood)", las=1)
#add time labels
...
```{r ex4_vd_c3, class.source="fold-show", warning=FALSE, message=FALSE}
# stage samples
...
To build `dshou2016`, *C. elegans* aging time-series from @hou2016

...
Along the explanations, examples will be given using *C. elegans* larval development time-series data published by @aeschimann2017lin41 and @hendriks2014extensive, stored in `dsaeschimann2017` and `dshendriks2014` respectively.  [Code to create these objects can be found at the end of this vignette](#code-to-generate-data-objects).

Finally, a few more examples of building references are given at the end for broader coverage of reference-building needs. 


# Selecting / Preparing a dataset

We need to start from a gene expression profiling time-series that spans the developmental stages of interest for your organism.
Thankfully, these time-series experiments are (increasingly) numerous in the literature and most model organisms will already have some kind of useable data. 
You may also make (or already have) your own time-series on hand.

## Databases
There are a few databases you can search for and download data from. 
...
Several points of the experimental design should be kept in mind when selecting data for a reference.
   ...
   * ***Are there replicates ?*** If so, good. This means you can confirm the dynamics in the data are not noise. A slightly sparser time-series with replicates could be a better candidate to build a reference than a higher-resolution experiment with one batch.
   * ***Is the time point sampling even ?*** Profiling is still expensive, so time courses experiments usually account for dynamic ranges of development (*i.e*, early or fast-changing stages are more sampled). We ideally want even sampling (which is better for spline fitting).  However, if a dataset's sampling respects dynamic ranges, you can still use it for interpolation as-is, or interpolate with a trick using ranks.
   * ***What's the developmental range ?*** The bigger, the better!
...
heatmap(cor_dsaeschimann2017[ord, ord], Colv = NA, Rowv = NA, 
        ...
        # see 'Useful functions' for transp() function.
...
boxplot(cor_dsaeschimann2017~interaction(dsaeschimann2017$p$strain, 
        ...
        col = transp(1:4), # see 'Useful functions' for transp() function.
...
Here you will find a few examples of reference building on different organisms.
 ...
 - [*C. elegans* larval development](#c.-elegans-larval-development)
 - [*D. melanogaster* embryonic development](#d.-melanogaster-embryonic-development) 
 - [*Danio rerio* embryonic development](#danio-rerio-embryonic-development)

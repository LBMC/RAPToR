...
g_mp <- mperf(ds1$g, predict(m_ds1), is.t = T)
g_mp
ng_mp <- mperf(ds1$g, predict(m_ds1), is.t = T, global = F)
```
...
invisible(sapply(names(ng_mp), function(idx){
  if(idx == "aCC"){
    d <- density(na.omit(ng_mp[[idx]]), from = 0, to = 1)
    pos <- 2
  } else {
    d <- density(na.omit(ng_mp[[idx]]))
    pos <- 4
  }

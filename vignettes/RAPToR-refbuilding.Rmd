...
This strategy is what we use in our article (@bulteau2022real).

A more robust approach is to look for genes that change consistently along aging between multiple independent time-series. 
This can for example be done with an ICA joining multiple datasets, selecting genes with high loadings on components which behave the same across all included datasets.
Of course, this requires multiple independent aging time-series for an organism of interest (the more, the better), which is rarely found.

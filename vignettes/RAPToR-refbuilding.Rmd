...
gen_figs <- T # wether to generate figures or read from RAPToR-refbuilding_files/figs

figpath <- "RAPToR-refbuilding_figs/"
...
png_custom <- function(figname, path = "", 
                       fig.width = 7, fig.height = 5, res = 72, ...){
...
In the midst of the explanations, examples will be given using the same `ds1` and `ds2` datasets as in the general usage vignette ( [you can check out how these were built at the end of the vignette](#code-gen_ds1-ds2) ).
...
It is possible to include a `by` argument to the `s()` function, but this essentialy corresponds to separate fits on the levels of the specified variable.
...
```{r mperf_ds1, results='markup', eval = gen_figs}
...
```{r bxp_mperf_ds1, echo = F, fig.height=5, fig.width=8}
show_fig(expr = {  
  par(mfrow = c(2,2))
...
```{r cv_ds1, results='markup', eval = gen_figs}
...
```{r plot_cv_ds1_show, eval = F}
plot(cv_ds1, names = paste0("bs = ", smooth_methods), outline = F,
     swarmargs = list(cex = .8))
```

```{r plot_cv_ds1, echo = F, fig.width=9, fig.height=8, fig.align='center'}
show_fig(expr = {
  plot(cv_ds1, names = paste0("bs = ", smooth_methods), outline = F,
       swarmargs = list(cex = .8))
...
```{r plot_cv_ds1k, echo = F, fig.width=12, fig.height=9, fig.align='center'}
show_fig(expr = {
  par(mar = c(7,4,3,1))
  plot(cv_ds1k, names = c("na", paste0("k=", ks, rep(c("", ", fx=T"), each = 4))), outline = F,
...
```{r ndat_mds1, results='markup', eval = gen_figs}
...
```{r pred_mds1_comp, eval = gen_figs}

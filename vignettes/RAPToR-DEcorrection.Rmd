...
plot(ae_miki, main = "Miki et al. (2017) samples", 
     ...
     l.pos = "topright", lmar = 10, g.line= 1, show.boot_estimates = F)
```
...
knitr::include_graphics("../inst/cdoc/RAPToR-DEcorrection_figs/refcompare_method.png")
```
...
In this case, over 75% of expression changes can be explained by development, which is expected given the large difference between WT-3 and the *xrn-2* samples.
...
plot(ae_dif, r_coef, type='b', ylim = c(-0.05,1.0),
     ...
     xlab = "Average age difference between groups (h)", lwd=2, las=2)
...
We then build a DESeq model including age and strain and test for significant differences between mutant and WT groups. We define a `run_DESeq2_age()` function to do this (see [*DE and reference functions* below for code](#code-de_functions)). 
...
We use [precision and recall (P/R)](https://en.wikipedia.org/wiki/Precision_and_recall) metrics to measure the performance the DE analysis (p-value) in discriminating truly DE genes (*i.e.* genes found in the gold-standard DE above) from non-DE genes. An area under the P/R curve of 1 would be a perfect classifier.
...
If we select DE genes with the same thresholds as the gold-standard, this drop in performance translates to both a decrease of true positives, and an increase of false positives.
...
Integrating reference data in the DE model complicates the analysis but makes it possible to recover truly DE genes when there is no age overlap between the sample groups to compare thanks to the reference bridging the gap.

Most DE analysis tools input *raw counts* due to their particular statistical properties, so the interpolated reference data must be converted from TPM to (artificial) counts assuming an arbitrary fixed library size ($25\times 10^6$ counts).
...
Note: this approach improves upon what is presented in the RAPToR article and generates valid p-values.
...
We select a reference window spanning the sample age range with an added 1 hour margin for each subset.  
...
We define a `find_df()` function which computes the residual sum of squares for a range of spline dfs and choose the optimal df values where a plateau is reached (see [*DE and reference functions* below for code](#code-de_functions)).
...
# optimal df for each subset
df_opti <- c(2,3,3,4,5,6) 
```
...
plot(range(dfs), range(dfs_ssq)+c(0,12), 
     ...
     xlab = 'spline df', ylab = "Residual SSQ", bty = 'l'); twoTicks()
invisible(sapply(seq_along(dfs_ssq), function(i) {
  points(dfs, 2*i+dfs_ssq[[i]], type = 'b', col = c('red',col.palette2)[i], lty=3, lwd=2)
  points(df_opti[i], 2*i+dfs_ssq[[i]][which(dfs==df_opti[i])], cex = 1.75, pch=1)
}))
...
## Reference and DE functions 

<a name="code-de_functions"></a>

Running DESeq2 standard workflow (`run_DESeq2_age()`) and extracting a results table (`get_DEres()`).
...
Finding optimal df (`find_df()`) and converting reference data to counts (`log1ptpm_2rawcounts()`, `ref_2counts()`)
...
run_DESeq2_ref <- function(X, p, formula, ref, 
  ...
  pj <- cbind(
    ...
    rbind(p2, do.call(cbind, lapply(lev0, rep, times=nR))) # other terms
    ) 
  ...
  s0 <- pj$Batch=="s"
  dds0 <- DESeqDataSetFromMatrix(countData = Xj[,ss],
                                 colData = pj[ss,],
                                 design = f0)

---
title: "Correcting for age in differential expression analysis"
output: 
...
The developmental speed of fast-growing organisms such as *C. elegans* is influenced by many different factors -- including experimental conditions themselves -- making it difficult or impossible to collect age-matched individuals between conditions. 
...
# Quickstart
```{r sc4_load_qs, include=FALSE}
load("../inst/extdata/sc4_qsdata.RData")
```

## Quantifying age-driven expression changes with `ref_compare()`

```{r sc4_qsq0.loadlib, class.source="fold-show", message=FALSE, warning=FALSE}
# load libraries
library(RAPToR)
library(wormRef)
```
...
```{r sc4_qsq1.1, class.source="fold-show", message=FALSE, warning=FALSE, results='hide'}
# load reference 
...
```
```{r sc4_qs1.2, class.source="fold-show", fig.height=3, fig.width=6, out.width="60%"}
...
```{r sc4_qsq2, class.source="fold-show"}
qs_rc <- ref_compare(
...
```{r sc4_qsq3.2, class.source="fold-show", fig.height=3.5, fig.width=3.5, out.width="40%"}
...
```{r sc4_qsq3.1, class.source="fold-show"}
print(qs_rc)
```

In this example, ctr *vs.* mut logFCs have a correlation of $`r round(cor(qs_lfc[,1], qs_lfc[,2]),3)`$ with the expected developmental logFCs computed from the reference data. 
Using $r^2$, this corresponds to approximately $`r round(100*(cor(qs_lfc[,1], qs_lfc[,2])^2),2)` \%$ of variance explained by development.
...
```{r sc4_qsq3.3, fig.height=3.5, fig.width=3.5, out.width="40%", class.source = 'fold-show'}
...
```{r sc4_qsc0.loadlib, class.source="fold-show", message=FALSE, warning=FALSE}
# load libraries
library(RAPToR)
library(wormRef)
...
First, infer sample age with RAPToR.
```{r sc4_qsq1.1, class.source="fold-show", eval=F}
```
```{r sc4_qsc1.2, class.source="fold-show", fig.height=3, fig.width=6, out.width="60%"}
plot(ae_qs, group = qs$pdat$strain, g.line=3, lmar=5)
```

Define a reference window to integrate.
```{r sc4_qsc2.1, class.source="fold-show"}
...
```{r sc4_qsc2.2, class.source="fold-show", fig.height=3.5, fig.width=5, out.width="40%"}
dfs <- 1:8
...
```{r sc4_qsc2.3, class.source="fold-show"}
# get gene lengths
...
```{r sc4_qsc3.1, class.source="fold-show"}
...
```{r sc4_qsc4.1, class.source="fold-show", warnings=F, message=FALSE}
# build DESeq object
...
```{r sc4_qsc4.2, class.source="fold-show", warning=FALSE, message=FALSE}
...
Extract DE results.
```{r sc4_qsc5.1, class.source="fold-show"}
res <- results(dd1)
summary(res)
```

```{r sc4_qsc5.3, class.source="fold-show", fig.height=3.5, fig.width=6, out.width="60%"}
DESeq2::plotMA(dd1)
```
...
Custom functions to transform reference data into artificial counts and run DESeq2 as described above are given in [*DE functions*, below](#de-functions) (please note they may require tweaking). 
The following is equivalent to the code above.

```{r sc4_qsc5.2, class.source="fold-show", eval=F}
dd1 <- run_DESeq2_ref(
...
Any correlation between observed (sample) and expected (reference) logFCs will then correspond to the developmental effect, with uncorrelated logFCs corresponding to the perturbation effect (**c**).
...
```{r sc4_qsq2, class.source="fold-show", eval=F}
```

The resulting sample and reference (log2) logFCs can be extracted with `get_logFC()` and plotted. 
When there are more than 2 compared conditions, you can specify the factor levels to compare to the control (with `l`) when extracting the logFCs.
...
By selecting specific samples of matching development from both time-series, we define a gold-standard of truly DE genes.
Then, by sliding the window of WT samples we can evaluate the effect of increasing developmental shifts between the groups on the DE analysis, as well as measure the advantages of integrating reference data in the model.
...
```{r sc4_deseq_wotage, echo=F, include=F}
...
```{r sc4_ndegenbp, echo=F, fig.height=5, fig.width=3, out.width="35%"}
par(mar=c(4,4,1,1))
...
```{r sc4_load_qsdata, echo=TRUE, eval=FALSE}
...
```{r sc4_sessInfo, collapse=FALSE, class.source = 'fold-show'}
sessionInfo()
```

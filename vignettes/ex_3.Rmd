...
The reference data (`dswhite2017`) has uneven time sampling, as can often be the case to account for differences in dynamic ranges of expression.
We can apply a transformation on the time values (using `asinh()` in this case) to render them more uniform and thus avoid interpolation bias. 
...
Because of this, fitting splines along age on these components poorly fit the earlier time points in particular.
...
Since sample batch is clearly absent from the components, we can exclude it for a more parsimonious model.
...
```
We can see that the model has trouble predicting the dynamics accurately.

PC3 and PC4 have flattened early dynamics, and overfitting around 40hpf.

```{r ex3_vd_c6, fig.width=7, fig.height=3.5, out.width="50%"}
...
```{r ex3_vd_c8, class.source="fold-show", warning=FALSE, message=FALSE}
...
The "gaps" or "steps" seen on the validation data estimates caused by interpolation bias (overfitting and poorly fit dynamics mentioned above).

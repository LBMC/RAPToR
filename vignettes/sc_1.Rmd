...
Quite some imprecise development !
...
```{r sc1_cor_dev_plot, fig.height=6, fig.width=8}
show_fig(expr = {
xa <- cc["wt_h06.3",]
...
DGE <- function(X, strain, age, df = 2, name = NULL){
  require(splines)
...
```{r do_dge, eval = gen_figs}
# format gdata
X <- log2(exp(dslehrbach2012$g))

dge.ca <- DGE(X = X, strain = dslehrbach2012$p$strain, age = dslehrbach2012$p$tpastL4, name = "dge.ca")
dge.ae <- DGE(X = X, strain = dslehrbach2012$p$strain, age = ae_dslb$age.estimates[,1], name = "dge.ae")
```
...
### Compare with random noise on chron. age

Let's compare the DGE results we have on the estimate ages with random perturbations of similar distribution.
If the asynchronicity observed with age estimates is erroneous, then using random noise around the chronological values should yield similar results to using estimates in the model.

```{r sc1_comp_dge1, eval = gen_figs}
age_diffs <- (dslehrbach2012$p$tpastL4 + 50) - ae_dslb$age.estimates[,1]
# Note : + 50 to shift tpastL4 to age and avoid negative ages,
#        this has no impact on the DGE.

# estimate density function of age_diff
d_ad <- density(age_diffs)

# generate age sets of with random age_diff-like noise 
set.seed(1)

n <- 100
rd_ages <- lapply(seq_len(n), function(i){
  dslehrbach2012$p$tpastL4 + sample(x = d_ad$x, size = nrow(dslehrbach2012$p), prob = d_ad$y, replace = T) 
})
...
hist(diffgens$mut, breaks = 50, main = "Nb of DE genes for strain")
...
points(qts, quantile(dge.ca$gof, probs = qts), type = "l", lwd = 4, col = 2)
plot_QQGoFbox(dge.ae$gof[ord], bins = 50, add = T, cex = .2, lwd = 2, boxcol=4, medcol = 4, medlwd=6, boxwex = .8/50)


plot(0:1, 0:1, type = "n", ylab = "GoF >= quantile", xlab = "quantile", main = "Goodness of Fit distribution")
points(qts, quantile(dge.ca$gof, probs = qts), type = "l", lwd = 4, col = 2)

...
If we haven't built your favorite organism's references yet, you can take a look at [how to build your own reference](#building-your-own-reference) for a quick-start guide or the [reference-building vignette](RAPTOR-refbuilding.html) for a more in-depth explanation.
...
Note that [there is a vignette entirely dedicated to this process](RAPTOR-refbuilding.html), so this section is just a quick overview.

Interpolating on time series data is the basis for building high-temporal-resolution references.
...
dfCV_ds2 <- df_CV(X = ds2$g, 
                  ...
                  nb.cores = 2)
```
```{r plot_dfCV, fig.width=6, fig.height=6, out.width="60%", eval = F}
...
It's alright if the prediction is not perfect, but we want the general dynamics to be accurate (oscillations, increasing or decreasing trends, *etc.*)

```{r plot_genes, fig.width=14, fig.height=7, eval = F}
set.seed(8)
...
```
```{r plot_ae_ds2_rds2, fig.height=6, fig.width=6, out.width="60%", eval = F}

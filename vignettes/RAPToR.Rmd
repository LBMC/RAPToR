...
#> dsaeschimann2017$p$age    1.50745    0.02462  61.223  < 2e-16 ***
#> ---
...
#> dshendriks2014$p$age    1.55868    0.02562  60.845  < 2e-16 ***
#> ---
#> Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
#> 
...
m_dsaeschimann2017 <- ge_im(X = dsaeschimann2017$g, p = dsaeschimann2017$p, formula = "X ~ s(age, bs = 'tp', k = 9, fx=TRUE) + strain", nc = 19)
```
...
Below is an example of usage to choose a degree of freedom for a spline on the `dsaeschimann2017` GEIM.
...
flist <- as.list(paste0("X ~ s(age, bs = \'tp\', k=", k, ", fx=TRUE) + strain"))
flist
#> [[1]]
...
plot(cv_dsaeschimann2017, names = paste0("k = ", k), outline = F,
...
From the plots above, choosing `k = 8` seems to be the best-performing choice (minimum CV error and plateau of model performance). 
...
show_fig(expr = {
  ...
  plot(ae_test_dsaeschimann2017$age.estimates[,1]~dsaeschimann2017$p$age, 
       ...
       xlim = rg, ylim = rg,
       main = "Chron. vs Estimated ages for dsaeschimann2017 (on dsaeschimann2017 reference)", lwd = 2, col = factor(dsaeschimann2017$p$strain))
  invisible(sapply(levels(factor(dsaeschimann2017$p$strain)), function(l){
    s <- dsaeschimann2017$p$strain == l
  ...
  plot(ae_test_dshendriks2014$age.estimates[,1]~dshendriks2014$p$age, 
       xlab = "Chronological age", ylab = "Estimated age (dsaeschimann2017)", 
       xlim = rg, ylim = rg,
       main = "Chron. vs Estimated ages for dshendriks2014 (on dsaeschimann2017 reference)", lwd = 2)
...
```{r c2tpm_func, code = readLines("vignettes/convert2tpm.R"), echo = TRUE, eval=FALSE}
```

```{r load_dsaeschimann2017_c, eval = F}
...
```{r load_dshendriks2014_c, eval = F}
...
X_dshendriks2014 <- RAPToR::format_ids(X_dshendriks2014, wormRef::Cel_genes, from = "wb_id", to = "wb_id")
X_dshendriks2014 <- raw2rpkm(X = X_dshendriks2014, gene.length = wormRef::Cel_genes, id.col = "wb_id", l.col = "transcript_length")

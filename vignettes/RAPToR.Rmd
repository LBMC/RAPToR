...
Unfortunately, this is only feasible for experiments with hundreds of samples and/or time-series designs, which are not yet the general standard of expression profiling studies. 
...
`RAPToR` allows you to estimate the developmental age of individual samples from their *gene expression profiles*.
...
Since we aimed to provide a way to predict the age of samples easily, we pre-built several references for commonly used organism from available data in the literature.
...
In this part, we'll be showcasing usage of `RAPToR` on two *C. elegans* time series datasets from the literature.
...
As the code to create the `dsaeschimann2017` and `dshendriks2014` objects is lengthy and not the object of this vignette, you can find it [at the end of the document](#load_dsaeschimann2017_dshendriks2014).
...
The only other transformation the data may need is to convert probe IDs or gene IDs to match those of the reference series. 

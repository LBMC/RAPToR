...
ds1$g <- log1p(ds1$g) # log1p(x) = log(x + 1)
...
par(mfrow=c(2,2))
plot_cor.ae(ae_ds2, subset = c(1,4,9,11))
```
...
m_ds1 <- ge_im(X = ds1$g, p = ds1$p, formula = "X ~ s(age, bs = 'tp', k = 8, fx=TRUE) + strain", nc = 19)
```
...
For example, on the `ds1` dataset, we set the threshold at $99\%$ :

```{r pca_ds1, eval = gen_figs}
pca_ds1 <- stats::prcomp(t(ds1$g), center = TRUE, scale = FALSE, rank = 25)
nc <- sum(summary(pca_ds1)$importance[3,] < .99) + 1
nc
#> [1] 19
```
...
Below is an example of usage to choose a degree of freedom for a spline on the `ds1` GEIM.
...
plot(cv_ds1, names = paste0("k = ", k), outline = F,
...
show_fig(expr = {
  ...
  invisible(sapply(seq_len(8), function(i){
    plot(ds1$p$age, pca_ds1$x[,i], lwd = 2, col = ds1$p$strain,
    ...
    sapply(seq_along(levels(ds1$p$strain)), function(l){
      ...
      points(ds1$p$age[s], pca_ds1$x[s,i], col = l, 
             type = 'l', lty = 2)
    })

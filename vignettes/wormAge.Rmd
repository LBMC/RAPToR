...
```{r prep_ref}
ref <- prepare_refdata("Cel_YA_adult1", n.inter = 600)
```
...
All are *C. elegans* time series :
 ...
 - `Cel_larval` holds larval development time series (time in hours post-hatching for 20$^\circ$C, aside from `time.series.25`) from @oudenaarden2013. This data holds 2 series one at 20$^\circ$C and one at 25. The data loaded with `prepare_refdata()` uses both, with the 25$^\circ$C series'time re-estimated

...
options(width=100)
quick_build <- TRUE # wether to cache heavy-computation chunks 
```
...
For our example, the `Cel_YA_adult1` dataset is appropriate since our samples go from late L3 to young adult. The `n.inter` corresponds to the resolution of the 
...
Let's quickly compare our estimates with the chronological time points.
...
time.series <- pheno.hendriks[sel, "age"]
nc <- 8 # nb of components
ica.hendriks <- ica::icafast(geno.hendriks[,sel], nc)
```
...
```{r interpol_hend, fig.height=5, fig.width=10}
par(mfrow=c(2,4))

sps <- c(.3,.5,.3,.4,.5,.4,.4,.4)
ref_hend <- interpol_refdata(X = geno.hendriks[,sel],
                             time.series = time.series,
